...
public class NodeMain {
    ...
    // Mesaj deposu - şimdilik in-memory, 2. Aşamada disk'e geçilecek
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                switch (cmd.getType()) {
                    case SET:
                        // Mesajı kaydet
                        messageStore.set(cmd.getMessageId(), cmd.getMessageText());
                        writer.println("OK");
                        System.out.println("✅ SET successful: id=" + cmd.getMessageId());
                        break;

                    case GET:
                        // Mesajı getir
                        String message = messageStore.get(cmd.getMessageId());
                        if (message != null) {
                            writer.println(message);
                            System.out.println("✅ GET successful: id=" + cmd.getMessageId());
                        } else {
                            writer.println("NOT_FOUND");
                            System.out.println("❌ GET failed: id=" + cmd.getMessageId() + " not found");
                        }
                        break;
